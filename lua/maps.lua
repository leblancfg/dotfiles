...
vim.api.nvim_set_keymap('n', '<F6>', ':let _s=@/ <Bar> :%s/\\s\\+$//e <Bar> :let @/=_s <Bar> :nohl <Bar> :unlet _s <CR>', {silent = true})
...
vim.api.nvim_set_keymap('n', '<Leader>z', ':w<CR> :AsyncRun pandoc % -o %:r.pdf --highlight-style zenburn --variable urlcolor=cyan && open %:r.pdf<CR>', {})
